...
std::exception SolidException(const std::string& id, const char* param = nullptr);
...
namespace SolidFramework { namespace Platform { namespace Plumbing {
    ...
    Logging::Logging()
    {
        ...
        if (failed)
        {
            throw SolidException("Failed to create mutex in Logging::Logging()");
        }
    }
    ...
    {
        if (!isLogging || loggingPath.empty())
        {
            return;
        }
      #if defined __SOLIDWINDOWS__
        ...
        {
            throw SolidException("Failed to get mutex in Logging::writeLine");
        }
        WIN32_FILE_ATTRIBUTE_DATA fInfo;
        GET_FILEEX_INFO_LEVELS fInfoLevelId = GetFileExInfoStandard;
        if (::GetFileAttributesEx(loggingPath.c_str(), fInfoLevelId, &fInfo))
        {
            if (fInfo.nFileSizeLow > rotationSize)
            {
                RotateLog(loggingPath);
            }
        }
        ...
        std::string strPath = converterX.to_bytes(loggingPath.c_str());
        struct stat myStat;
        if (::stat(strPath.c_str(), &myStat) == 0)
        {
            if (myStat.st_size > rotationSize)
            {
                RotateLog(loggingPath);
            }
        }
...
std::exception SolidException(const std::string& id, const char* param /* = nullptr */)
{
    ...
    if (id == "InvalidLicense" || id == "BadLicenseXML" || id == "LicenseExpired" || id == "LicenseViolation")
    {
    ...
    if (id == "ReadonlyObject" || id == "DocumentMustBeOpen" || id == "DocumentAlreadyOpen" || 
        ...
        id == "DontUsePagesModelAfterClose" || id == "DontUseNativeDocumentAfterClose")
    {
...
{
    throw SolidException(id, paramName);
}
...
namespace SolidFramework {
    ...
    void FrameworkObject::Wrap(HANDLE cPtr, bool ownsMemory)
    {
        if (cPtr == nullptr) throw std::invalid_argument("HANDLE cannot be null"); // never good
        this->cPtr = cPtr;
...
static void InitializeCallbacks() {
    static ConnectType_RegisterStringCallback registerStringCallback = GetFunction<ConnectType_RegisterStringCallback>("SWIGRegisterStringCallback_SolidFrameworkNative");
    static ConnectType_Exceptions registerExceptionsCallback = GetFunction<ConnectType_Exceptions>("SWIGRegisterExceptionCallbacks_SolidFrameworkNative");
    static ConnectType_SolidExceptions registerSolidExceptionsCallback = GetFunction<ConnectType_SolidExceptions>("RegisterSolidExceptionsCallbacks");
    static ConnectType_ArgumentExceptions registerArgumentExceptionsCallback = GetFunction<ConnectType_ArgumentExceptions>("SWIGRegisterExceptionArgumentCallbacks_SolidFrameworkNative");
...
public:
    ...
    public:
        TemporaryWrapper(HANDLE cPtr, bool memoryOwn, InvokeType_VH destructor) {
            this->cPtr = cPtr;
    ...
    template<typename Type> static std::vector<std::shared_ptr<Type>> CreateVectorFromPtrCollection(HANDLE cPtr, bool memoryOwn, InvokeType_UH getSize, InvokeType_HHI getItem, InvokeType_VH destructor) {
        ...
        for (unsigned int i = 0; i < size; i++) {
            ...
            vector[i] = newCPtr == nullptr? nullptr : std::make_shared<Type>(newCPtr, memoryOwn);
        }
        if (memoryOwn) {
            destructor(cPtr);
        }
    ...
    static TemporaryWrapperPtr VectorToOcrWordsCollection(const std::vector<SolidFramework::Imaging::OcrWordPtr>& vector) {
        ...
        if (cPtr == nullptr) throw SolidException("Failed to create collection");
...
namespace SolidFramework { 
    ...
    PageRange::PageRange(std::vector<int> range) {
        static InvokeType_HH proc = GetFunction<InvokeType_HH>("new_SolidFramework_PageRange__SWIG_3");
        HANDLE cPtr = proc(SolidFrameworkNative::VectorToPagesArray(range)->GetCPtr());
        Wrap(cPtr, true);
    }
    ...
    PageRange::PageRange(Plumbing::DocumentPtr document) {
        ...
        HANDLE cPtr = proc(FrameworkObject::GetCPtr(document));
        Wrap(cPtr, true);
    }
    ...
    PageRange::PageRange(Plumbing::DocumentPtr document, Plumbing::PageRanges pageRangeSet) {
        ...
        HANDLE cPtr = proc(FrameworkObject::GetCPtr(document), (int)(pageRangeSet));
        Wrap(cPtr, true);
    }
    ...
    PdfAWarningEventArgs::PdfAWarningEventArgs(std::wstring message) {
        ...
        HANDLE cPtr = proc(message.c_str(), (HANDLE)this);
        Wrap(cPtr, true);
    }
    ...
    ProgressEventArgs::ProgressEventArgs(int progress, int maxProgress, Interop::SolidErrorCodes statusCode, std::wstring statusDescription) {
        ...
        HANDLE cPtr = proc(progress, maxProgress, (int)(statusCode), statusDescription.c_str(), (HANDLE)this);
        Wrap(cPtr, true);
    }
...
namespace SolidFramework { namespace Converters { 
    ...
    void Converter::AddSourceFiles(std::vector<std::wstring> paths) {
    ...
    PdfToDataConverter::PdfToDataConverter() {
        ...
        directorConnect(cPtr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &SolidFrameworkNative::SolidFramework_Converters_Converter_HandleProgress, &SolidFrameworkNative::SolidFramework_Converters_Converter_HandleWarning, nullptr);
        Wrap(cPtr, true);
        SolidFrameworkNative::SolidFramework_Converters_Converter_SetCustomData(cPtr, (HANDLE)this);
    }
    ...
    PdfToPdfConverter::PdfToPdfConverter() {
        ...
        directorConnect(cPtr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &SolidFrameworkNative::SolidFramework_Converters_Converter_HandleProgress, &SolidFrameworkNative::SolidFramework_Converters_Converter_HandleWarning, nullptr, nullptr);
        Wrap(cPtr, true);
        SolidFrameworkNative::SolidFramework_Converters_Converter_SetCustomData(cPtr, (HANDLE)this);
    }
...
namespace SolidFramework { namespace Converters { namespace Plumbing { 
    ConversionResult::ConversionResult(std::wstring source, ConversionStatus status) {
        ...
        HANDLE cPtr = proc(source.c_str(), (int)(status));
        Wrap(cPtr, true);
    }
    ...
    void ConversionResult::SetPaths(std::vector<std::wstring> value) {
    ...
    ConversionStatus ConversionResult::GetStatus() {
        static InvokeType_IH proc = GetFunction<InvokeType_IH>("SolidFramework_Converters_Plumbing_ConversionResult_Status_get");
        return (ConversionStatus)proc(cPtr);
    }
    
    void ConversionResult::SetStatus(ConversionStatus value) {
        static InvokeType_VHI proc = GetFunction<InvokeType_VHI>("SolidFramework_Converters_Plumbing_ConversionResult_Status_set");
        proc(cPtr, (int)(value));
    }
    ...
    void IConversionResult::SetPaths(std::vector<std::wstring> value) {
    ...
    ConversionStatus IConversionResult::GetStatus() {
        static InvokeType_IH proc = GetFunction<InvokeType_IH>("SolidFramework_Converters_Plumbing_IConversionResult_Status_get");
        return (ConversionStatus)proc(cPtr);
    }
    
    void IConversionResult::SetStatus(ConversionStatus value) {
        static InvokeType_VHI proc = GetFunction<InvokeType_VHI>("SolidFramework_Converters_Plumbing_IConversionResult_Status_set");
        proc(cPtr, (int)(value));
    }
    ...
    SelectedArea::SelectedArea(int page, int x, int y, int width, int height) {
        ...
        HANDLE cPtr = proc(page, x, y, width, height);
        Wrap(cPtr, true);
    }
    ...
    SelectedArea::SelectedArea(int page, SolidFramework::Interop::PointPtr location, int width, int height) {
        ...
        HANDLE cPtr = proc(page, SolidFramework::FrameworkObject::GetCPtr(location), width, height);
        Wrap(cPtr, true);
    }
    ...
    SelectedArea::SelectedArea(int page, SolidFramework::Interop::RectanglePtr rect) {
        ...
        HANDLE cPtr = proc(page, SolidFramework::FrameworkObject::GetCPtr(rect));
        Wrap(cPtr, true);
    }
...
namespace SolidFramework { namespace Imaging { 
    ...
    Ocr::Ocr(std::wstring languageCode) {
        ...
        HANDLE cPtr = proc(languageCode.c_str());
        Wrap(cPtr, true);
    }
    ...
    OcrLine::OcrLine(std::vector<SolidFramework::Imaging::OcrWordPtr> words) {
        static InvokeType_HH proc = GetFunction<InvokeType_HH>("new_SolidFramework_Imaging_OcrLine");
        HANDLE cPtr = proc(SolidFrameworkNative::VectorToOcrWordsCollection(words)->GetCPtr());
        Wrap(cPtr, true);
    }
...
namespace SolidFramework { namespace Interop { 
    ...
    Bitmap::Bitmap(int width, int height, ImageType type) {
        ...
        HANDLE cPtr = proc(width, height, (int)(type));
        Wrap(cPtr, true);
    }
    ...
    void Bitmap::SetPalette(std::vector<SolidFramework::Interop::ColorPtr> value) {
    ...
    DateTime::DateTime(int year, int month, int day, int hour, int minute, int second, bool isLocal) {
        ...
        HANDLE cPtr = proc(year, month, day, hour, minute, second, isLocal);
        Wrap(cPtr, true);
    }
    ...
    DateTime::DateTime(int year, int month, int day, int hour, int minute, int second) {
        ...
        HANDLE cPtr = proc(year, month, day, hour, minute, second);
        Wrap(cPtr, true);
    }
    
    DateTime::DateTime(SolidFramework::Interop::DateTimePtr date) {
        static InvokeType_HH proc = GetFunction<InvokeType_HH>("new_SolidFramework_Interop_DateTime__SWIG_3");
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(date));
        Wrap(cPtr, true);
    }
    ...
    Matrix::Matrix(double A, double B, double C, double D, double Tx, double Ty) {
        ...
        HANDLE cPtr = proc(A, B, C, D, Tx, Ty);
        Wrap(cPtr, true);
    }
    ...
    PointF::PointF(SolidFramework::Interop::PointFPtr value) {
        static InvokeType_HH proc = GetFunction<InvokeType_HH>("new_SolidFramework_Interop_PointF__SWIG_1");
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(value));
        Wrap(cPtr, true);
    }
    ...
    PointF::PointF(float x, float y) {
        ...
        static InvokeType proc = GetFunction<InvokeType>("new_SolidFramework_Interop_PointF__SWIG_2");
        HANDLE cPtr = proc(x, y);
        Wrap(cPtr, true);
    }
    ...
    Rectangle::Rectangle(SolidFramework::Interop::RectanglePtr value) {
        static InvokeType_HH proc = GetFunction<InvokeType_HH>("new_SolidFramework_Interop_Rectangle__SWIG_1");
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(value));
        Wrap(cPtr, true);
    }
    ...
    Rectangle::Rectangle(int left, int top, int right, int bottom) {
        ...
        static InvokeType proc = GetFunction<InvokeType>("new_SolidFramework_Interop_Rectangle__SWIG_2");
        HANDLE cPtr = proc(left, top, right, bottom);
        Wrap(cPtr, true);
    }
    ...
    RectangleF::RectangleF(SolidFramework::Interop::RectangleFPtr value) {
        static InvokeType_HH proc = GetFunction<InvokeType_HH>("new_SolidFramework_Interop_RectangleF__SWIG_1");
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(value));
        Wrap(cPtr, true);
    }
    ...
    RectangleF::RectangleF(double left, double top, double right, double bottom) {
        ...
        static InvokeType proc = GetFunction<InvokeType>("new_SolidFramework_Interop_RectangleF__SWIG_2");
        HANDLE cPtr = proc(left, top, right, bottom);
        Wrap(cPtr, true);
    }
    ...
    SizeF::SizeF(SolidFramework::Interop::SizeFPtr value) {
        static InvokeType_HH proc = GetFunction<InvokeType_HH>("new_SolidFramework_Interop_SizeF__SWIG_1");
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(value));
        Wrap(cPtr, true);
    }
    ...
    SizeF::SizeF(float x, float y) {
        typedef HANDLE (SOLIDCALLBACK *InvokeType)(float, float);
        static InvokeType proc = GetFunction<InvokeType>("new_SolidFramework_Interop_SizeF__SWIG_2");
        HANDLE cPtr = proc(x, y);
        Wrap(cPtr, true);
    }
    ...
    TimeSpan::TimeSpan(int hour, int minute, int second) {
        ...
        HANDLE cPtr = proc(hour, minute, second);
        Wrap(cPtr, true);
    }
...
namespace SolidFramework { namespace Model { 
    ...
    PdfOptions::PdfOptions() {
        ...
        directorConnect(cPtr, nullptr, &SolidFrameworkNative::SolidFramework_Model_PdfOptions_HandleProgress, &SolidFrameworkNative::SolidFramework_Model_PdfOptions_HandleWarning);
        Wrap(cPtr, true);
        static InvokeType_VHH setCustomData = GetFunction<InvokeType_VHH>("SolidFramework_Model_PdfOptions_SetCustomData");
        setCustomData(cPtr, (HANDLE)this);
    }
...
namespace SolidFramework { namespace Model { namespace Pdf { namespace Pages { 
    ...
    ModelChangedEventArgs::ModelChangedEventArgs(ModelChangedEvent theEvent) {
        ...
        HANDLE cPtr = proc((int)(theEvent), (HANDLE)this);
        Wrap(cPtr, true);
    }
    ...
    bool Operation::GetEnabled() {
        static InvokeType_UH proc = GetFunction<InvokeType_UH>("SolidFramework_Model_Pdf_Pages_Operation_Enabled_get");
        return 0 != proc(cPtr);
    }
    ...
    ModelOperation Operation::GetModelOperation() {
        static InvokeType_IH proc = GetFunction<InvokeType_IH>("SolidFramework_Model_Pdf_Pages_Operation_ModelOperation_get");
    ...
    PagesChangedEventArgs::PagesChangedEventArgs(OperationPtr operation) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(operation), (HANDLE)this);
        Wrap(cPtr, true);
    }
    ...
    PageSelection::PageSelection(IPageHolderPtr page, SolidFramework::Interop::RectangleFPtr area) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(page), SolidFramework::FrameworkObject::GetCPtr(area));
        Wrap(cPtr, true);
    }
    ...
    void PagesModel::InsertRange(int position, std::vector<SolidFramework::Model::Pdf::Pages::IPageSourcePtr> collection) {
    ...
    void PagesModel::InsertRange(int insert, std::vector<SolidFramework::Model::Pdf::Pages::IPageHolderPtr> collection) {
    ...
    void PagesModel::InsertRange(int position, std::vector<std::wstring> pageFiles, bool preferPdfAWhenCreating) {
    ...
    void PagesModel::SetPageSelections(std::vector<SolidFramework::Model::Pdf::Pages::PageSelectionPtr> value) {
    ...
    void PagesModel::SetSelection(std::vector<int> value) {
    ...
    void PagesModel::ValidateIndices(std::vector<int> indices) {
    ...
    PasswordEnteredEventArgs::PasswordEnteredEventArgs(std::wstring password) {
        ...
        HANDLE cPtr = proc(password.c_str(), (HANDLE)this);
        Wrap(cPtr, true);
    }
    ...
    Plumbing::CommentPtr PdfPageHolder::AddHighlightComment(std::vector<SolidFramework::Interop::PointFPtr> location, SolidFramework::Interop::ColorPtr color, std::wstring title, std::wstring content) {
    ...
    Plumbing::CommentPtr PdfPageHolder::AddStrikeOutComment(std::vector<SolidFramework::Interop::PointFPtr> location, SolidFramework::Interop::ColorPtr color, std::wstring title, std::wstring content) {
    ...
    Plumbing::CommentPtr PdfPageHolder::AddTextComment(std::vector<SolidFramework::Interop::PointFPtr> location, SolidFramework::Interop::ColorPtr color, std::wstring title, std::wstring content) {
    ...
    Plumbing::CommentPtr PdfPageHolder::AddUnderlineComment(std::vector<SolidFramework::Interop::PointFPtr> location, SolidFramework::Interop::ColorPtr color, std::wstring title, std::wstring content) {
    ...
    bool PdfPageHolder::ModifyComment(Plumbing::CommentPtr comment, std::vector<SolidFramework::Interop::PointFPtr> newLocation, SolidFramework::Interop::ColorPtr newColor, std::wstring newTitle, std::wstring newContent) {
...
namespace SolidFramework { namespace Model { namespace Pdf { namespace Pages { namespace Operations { 
    ...
    AddCommentParameter::AddCommentParameter(SolidFramework::Model::Pdf::Pages::IPageHolderPtr page, SolidFramework::Model::Pdf::Pages::Plumbing::AnnotationType type, std::vector<SolidFramework::Interop::PointFPtr> position, SolidFramework::Interop::ColorPtr color, std::wstring title, std::wstring content) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(page), (int)(type), SolidFrameworkNative::VectorToPointFsCollection(position)->GetCPtr(), SolidFramework::FrameworkObject::GetCPtr(color), title.c_str(), content.c_str());
        Wrap(cPtr, true);
    }
    ...
    AddCommentParameter::AddCommentParameter(SolidFramework::Model::Pdf::Pages::Plumbing::CommentPtr parentComment, std::wstring title, std::wstring content) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(parentComment), title.c_str(), content.c_str());
        Wrap(cPtr, true);
    }
    ...
    SolidFramework::Model::Pdf::Pages::Plumbing::AnnotationType AddCommentParameter::GetType() {
        static InvokeType_IH proc = GetFunction<InvokeType_IH>("SolidFramework_Model_Pdf_Pages_Operations_AddCommentParameter_GetType");
    ...
    DeleteCommentParameter::DeleteCommentParameter(SolidFramework::Model::Pdf::Pages::Plumbing::CommentPtr comment) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(comment));
        Wrap(cPtr, true);
    }
    ...
    InsertParameter::InsertParameter(int position, SolidFramework::Model::Pdf::Pages::IPageSourcePtr source) {
        ...
        HANDLE cPtr = proc(position, SolidFramework::FrameworkObject::GetCPtr(source));
        Wrap(cPtr, true);
    }
    
    InsertParameter::InsertParameter(int position, std::vector<SolidFramework::Model::Pdf::Pages::IPageSourcePtr> sources) {
        ...
        HANDLE cPtr = proc(position, SolidFrameworkNative::VectorToIPageSourcesCollection(sources)->GetCPtr());
        Wrap(cPtr, true);
    }
    ...
    int InsertParameter::GetPosition() {
        static InvokeType_IH proc = GetFunction<InvokeType_IH>("SolidFramework_Model_Pdf_Pages_Operations_InsertParameter_GetPosition");
        return proc(cPtr);
    }
    ...
    ModifyCommentParameter::ModifyCommentParameter(SolidFramework::Model::Pdf::Pages::Plumbing::CommentPtr modifiedComment, std::vector<SolidFramework::Interop::PointFPtr> position, SolidFramework::Interop::ColorPtr color, std::wstring title, std::wstring content) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(modifiedComment), SolidFrameworkNative::VectorToPointFsCollection(position)->GetCPtr(), SolidFramework::FrameworkObject::GetCPtr(color), title.c_str(), content.c_str());
        Wrap(cPtr, true);
    }
    
    ModifyCommentParameter::ModifyCommentParameter(SolidFramework::Model::Pdf::Pages::Plumbing::CommentPtr modifiedComment, std::vector<SolidFramework::Interop::PointFPtr> position) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(modifiedComment), SolidFrameworkNative::VectorToPointFsCollection(position)->GetCPtr());
        Wrap(cPtr, true);
    }
    ...
    ModifyCommentParameter::ModifyCommentParameter(SolidFramework::Model::Pdf::Pages::Plumbing::CommentPtr modifiedComment, SolidFramework::Interop::ColorPtr color) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(modifiedComment), SolidFramework::FrameworkObject::GetCPtr(color));
        Wrap(cPtr, true);
    }
    ...
    ModifyCommentParameter::ModifyCommentParameter(SolidFramework::Model::Pdf::Pages::Plumbing::CommentPtr modifiedComment, std::wstring title, std::wstring content) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(modifiedComment), title.c_str(), content.c_str());
        Wrap(cPtr, true);
    }
...
namespace SolidFramework { namespace Pdf { namespace Plumbing { 
    PdfAction::PdfAction(SolidFramework::Pdf::PdfDocumentPtr pdfDocument) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(pdfDocument));
        Wrap(cPtr, true);
    }
    ...
    PdfDictionaryItemsCollection::PdfDictionaryItemsCollection(PdfDictionaryItemsCollectionPtr other) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(other));
        Wrap(cPtr, true);
    }
    ...
    PdfRectangle::PdfRectangle(SolidFramework::Pdf::PdfDocumentPtr pdfDocument, double x, double y, double width, double height) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(pdfDocument), x, y, width, height);
        Wrap(cPtr, true);
    }
    ...
    std::wstring PdfString::GetRawValue() {
        static InvokeType_SH proc = GetFunction<InvokeType_SH>("SolidFramework_Pdf_Plumbing_PdfString_GetRawValue");
        return UnwrapWString(proc(cPtr));
    }
    
    void PdfString::SetRawValue(std::wstring value) {
        static InvokeType_VHS proc = GetFunction<InvokeType_VHS>("SolidFramework_Pdf_Plumbing_PdfString_SetRawValue");
        proc(cPtr, value.c_str());
    }
...
namespace SolidFramework { namespace Pdf { namespace Transformers { 
    ...
    ImageWatermarkTransformer::ImageWatermarkTransformer(SolidFramework::Converters::Plumbing::ImageWatermarkPtr watermark) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(watermark));
        Wrap(cPtr, true);
    }
    ...
    PasswordSecurityTransformer::PasswordSecurityTransformer(std::wstring ownerPassword, SolidFramework::Pdf::EncryptionAlgorithm algorithm) {
        ...
        HANDLE cPtr = proc(ownerPassword.c_str(), (int)(algorithm));
        Wrap(cPtr, true);
    }
    ...
    PasswordSecurityTransformer::PasswordSecurityTransformer(std::wstring ownerPassword, std::wstring userPassword, SolidFramework::Pdf::EncryptionAlgorithm algorithm, SolidFramework::Pdf::AccessPermissions permission) {
        ...
        HANDLE cPtr = proc(ownerPassword.c_str(), userPassword.c_str(), (int)(algorithm), (int)(permission));
        Wrap(cPtr, true);
    }
    ...
    TransformationResult::TransformationResult(SolidFramework::Pdf::PdfDocumentPtr doc, SolidFramework::Interop::NativeError stat) {
        ...
        HANDLE cPtr = proc(SolidFramework::FrameworkObject::GetCPtr(doc), (int)(stat));
        Wrap(cPtr, true);
    }
...
namespace SolidFramework { namespace Platform { 
    ...
    bool Platform::SetAppDataDirectory(std::wstring path) {
        typedef unsigned int (SOLIDCALLBACK *InvokeType)(const wchar_t*);
        static InvokeType proc = GetFunction<InvokeType>("SolidFramework_Platform_Platform_SetAppDataDirectory");
        return 0 != proc(path.c_str());
    }
    ...
    bool Platform::SetCMapDirectory(std::wstring path) {
        typedef unsigned int (SOLIDCALLBACK *InvokeType)(const wchar_t*);
        static InvokeType proc = GetFunction<InvokeType>("SolidFramework_Platform_Platform_SetCMapDirectory");
        return 0 != proc(path.c_str());
    }
    ...
    bool Platform::SetColorProfileDirectory(std::wstring path) {
        typedef unsigned int (SOLIDCALLBACK *InvokeType)(const wchar_t*);
        static InvokeType proc = GetFunction<InvokeType>("SolidFramework_Platform_Platform_SetColorProfileDirectory");
        return 0 != proc(path.c_str());
    }
    ...
    bool Platform::SetFontDirectory(std::wstring path) {
        typedef unsigned int (SOLIDCALLBACK *InvokeType)(const wchar_t*);
        static InvokeType proc = GetFunction<InvokeType>("SolidFramework_Platform_Platform_SetFontDirectory");
        return 0 != proc(path.c_str());
    }
    ...
    bool Platform::SetFontsDataBaseFile(std::wstring path) {
        typedef unsigned int (SOLIDCALLBACK *InvokeType)(const wchar_t*);
        static InvokeType proc = GetFunction<InvokeType>("SolidFramework_Platform_Platform_SetFontsDataBaseFile");
        return 0 != proc(path.c_str());
    }
